...
function recordPromptStartTime(sessionId: string): void {
  // Record start time for duration tracking
  const startFile = `/tmp/pai-prompt-start-${sessionId}.txt`;
  Bun.write(startFile, Date.now().toString());
}
...
async function main() {
  try {
    ...
    // Record start time for cost/duration tracking
    if (payload.session_id) {
      recordPromptStartTime(payload.session_id);
    }
